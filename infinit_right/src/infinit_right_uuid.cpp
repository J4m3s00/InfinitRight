...
namespace IR {
	...
	IRUUID::IRUUID(u32 a, u32 b, u32 c, u32 d)
	{
		fData[0] = (u8)(a & 0xFF);
		fData[1] = (u8)((a >> 8) & 0xFF);
	...
	IRUUID::IRUUID(const IRUUID& src)
	{
		for (int i = 0; i < 16; i++)
			fData[i] = src.fData[i];
	}
	...
	IRUUID& IRUUID::operator=(const IRUUID& src)
	{
		for (int i = 0; i < 16; i++)
			fData[i] = src.fData[i];
		return *this;
	...
	bool IRUUID::operator==(const IRUUID& id) const
	{
		for (int i = 0; i < 16; i++) {
...
#pragma warning (disable: 4333)
	...
	void IRUUID::GetIRUUID(u32& out1, u32& out2, u32& out3, u32& out4) const
	{
	...
	bool IRUUID::operator < (const IRUUID& iid) const
	{
		bool	isLess = true;
		bool	isEqual = true;
		...
		iid.GetIRUUID(iida, iidb, iidc, iidd);
	...
	{
		IR_ASSERT(piece <= 0x0F, "Failed to get piece of IRUUID");
		char	ch;
		if (piece <= 9)
			ch = '0' + piece;
		else
	...
	static IRString GetPieceAsText(u8 piece)
	{
		char	buffer[3];
	...
	IRString IRUUID::ToString() const
	{
		IRString str;
		str += "{";
	...
	bool IRUUID::FromString(const IRString& str)
	{
		...
		if (!b)
		{
			// create new IRUUID if this is not a valid IRUUID
			IRUUID newIRUUID;
			*this = newIRUUID;
		}
